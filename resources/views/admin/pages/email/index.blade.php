        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="email-rightbar mb-3">
                    <div class="card">
                        <form action="{{ route('admin.email.mark') }}" method="post">
                            @csrf
                        <ul class="message-list">
                            <div class="btn-toolbar gap-2 p-3" role="toolbar">
                                <div class="btn-group">
                                    @if (Route::currentRouteName() == 'admin.email.index'  || Route::currentRouteName() == 'admin.email.allSentEmail' || Route::currentRouteName() == 'admin.email.allUnreadEmail' )
                                    <button  name='submitType' value="softDelete" type="submit" class="btn btn-primary waves-light waves-effect"><i
                                        class="far fa-trash-alt"></i>
                                    </button>
                                    @endif
                                    @if (Route::currentRouteName() == 'admin.email.trash' )
                        ...
                        @if (Route::currentRouteName() == 'admin.email.index' ||  Route::currentRouteName()=='admin.email.allUnreadEmail' )
                        ...
                        <li class="{{ $email->status=="unread" ?'unread':' '}}">
                            <div class="col-mail col-mail-1">
                                ...
                                <a href="{{ route('admin.email.showSpecificMail',$email->slug)}}" class="title">{{$email->EmployeeSender->name }}

                                </a>
                                @endif
                                 @if ($email->form)
                                <a href="{{ route('admin.email.showSpecificMail',$email->slug)}}" class="title">{{$email->sender->name }}
                                </a>
                                @endif
                            </div>
                            ...
                            <div class="col-mail col-mail-2">
                                @if($email->employee_form)
                                <a href={{ route('admin.email.showSpecificMail',$email->slug)}} class="subject">
                 ...
                 @endforeach
                 {{--  ||  Route::currentRouteName()=='admin.email.trash'  --}}
                         @elseif(Route::currentRouteName() == 'admin.email.allSentEmail' )
                         @foreach ($allEmailInfo['emailInfo'] as $email )
                         <li>
                             <div class="col-mail col-mail-1">
                                 <div class="checkbox-wrapper-mail">
                                 ...
                                 <a href="{{route('admin.email.showSpecificMail',$email->slug)}}" class="title">To :
                                     @if ($email->receiver)
                         ...
                         @elseif (Route::currentRouteName()=='admin.email.trash' )
                         @foreach ($allEmailInfo['emailInfo'] as $email )
                         <li>
                             <div class="col-mail col-mail-1">
                                 <div class="checkbox-wrapper-mail">
                                 ...
                                 <a href="{{route('admin.email.showSpecificMail',$email->slug)}}" style="pointer-events: none" class="title">To :
                                     @if ($email->receiver)
                                             {{ $email->receiver->name  ==   Auth::guard('admin')->user()->name ? 'me' : $email->receiver->name }}
                                     @endif
                                     @if ($email->EmployeeReceiver)
                             ...
                             <div class="col-mail col-mail-2">
                                 <a  style="pointer-events: none"  href="{{ route('admin.email.trash') }}" class="subject">
                                     <span class="bg-{{ $email->status == 'unread' ? "danger":"success"}} badge me-2">{{ $email->status }}</span>
                                     <span class="teaser">
                                     {!! htmlspecialchars_decode(strip_tags($email->description))!!}
                                 </span>
