                ...
                <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <h5 class="card-title me-4" style="float:left;margin-top:5px">Total Work Reports <span class="text-muted fw-normal ms-2">({{ count($workReports->toArray()) }})</span></h5>
                                ...
                                <div class="col-md-6">
                                    <div class="d-flex flex-wrap align-items-center justify-content-end gap-2 mb-3">
                                        <div id="category_button">
                                            <a href="{{ route('admin.workReport.index') }}" class="btn btn-sm btn-primary mb-2">All</a>
                                            <a href="{{ route('admin.workReport.trash') }}"  class="btn btn-sm mb-2 btn-danger">Trash</a>
                                    ...
                                    <tbody>
                                        @foreach ($workReports as $report)
                                            <tr>
                                                ...
                                                <td>
                                                    @if (Route::currentRouteName() == 'admin.workReport.index')
                                                        @if (Auth::guard('admin')->User()->can('work.report.status.edit') && !$report->deleted_at)
                                                        ...
                                                        @if (Auth::guard('admin')->User()->can('work.report.delete') &&!$report->deleted_at)
                                                            <a href="{{ route('admin.workReport.destroy', $report->id) }}"  class="btn btn-sm btn-danger"> <i class="fas fa-trash-alt"></i></a>
                                                        @endif
                                                        <a href="{{ route('admin.workReport.show',$report->id) }}"  class="btn btn-sm btn-info"> <i class="fas fa-eye"></i></a>
                                                    @endif

                                                    @if(Route::currentRouteName() == 'admin.workReport.trash')
                                                        @if (Auth::guard('admin')->User()->can('work.report.restore'))
                                                            <a href="{{ route('admin.workReport.restore', $report->id) }}" class="btn btn-sm btn-primary"><i class="fas fa-trash-restore" ></i></a>
                                                        @endif
                                                        @if (Auth::guard('admin')->User()->can('work.report.parmanentDelete'))
                                                    ...
                                                    <div class="modal fade" id="staticBackdrop{{ $report->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                                            <form action="{{ route('admin.workReport.update',$report->id) }}" method="POST">
                                                                @csrf
             ...
             $('.sweet_delete').click(function(){
                 ...
                 Swal.fire({
                   ...
                   if (result.isConfirmed) {
                       ...
                       $.ajax({
                          type:"GET",
                          url:'/admin/work-report/parmanent-delete/'+delete_id,
                          data: data,
