...
class EmailRepository{
     ...
     * @return  $allEmailInfo
     */
    public  function allAdmin($email){
        return Admin::select('id','email')->where('email','!=',$email)->get();
    }
    /**
     * get all employee
     */
    ...
    public function countTotalEmail(){
        return count( Email::with(['sender','receiver','EmployeeReceiver','EmployeeSender'])->where('to',
        Auth::guard('admin')->user()->id
        )->whereNull('soft_deleted_by_receiver')->latest()->get());
    }
    ...
    public function findEmail($id){
        ...
        $emails = Email::with(['sender','receiver','EmployeeReceiver','EmployeeSender'])->where('form',$id)->where('to',Auth::guard('admin')->user()->id)->whereNull('soft_deleted_by_receiver')->get();
        $allEmailInfo = [
    ...
    public function findSpecificEmail($slug){
        ...
        if( $email->status == "unread" && $email->to == Auth::guard('admin')->user()->id  ){
   ...
   /**
    ...
    public  function allUnreadEmail(){
        $allEmails = Email::with(['sender','receiver','EmployeeReceiver','EmployeeSender']) -> where('to',
            Auth::guard('admin')->user()->id
    ...
    public function countTrashEmail(){
        ...
        ->where(function($query){
            return $query->where('form',Auth::guard('admin')->User()->id)
            ->where('soft_deleted_by_sender','!=',' ')
            ->where('permanent_deleted_by_sender',null);
        })
        ->orWhere(function($query){
            return $query->where('to',Auth::guard('admin')->User()->id)->where('soft_deleted_by_receiver','!=',' ')->where('permanent_deleted_by_receiver',null);
        })
        ->get();
   ...
   /**
    ...
    public  function allSentEmail(){
        $allSentEmails = Email::with(['sender','receiver','EmployeeReceiver','EmployeeSender'])->where('form',
        Auth::guard('admin')->user()->id
    ...
    public function restore($id){
       ...
       if($email->form == Auth::guard('admin')->user()->id){
           $email->soft_deleted_by_sender = null;
       }
       else if($email->to == Auth::guard('admin')->user()->id){
   ...
   /**
    ...
    public  function allEmail(){
        $allEmails = Email::with(['sender','receiver','EmployeeReceiver','EmployeeSender'])->where(function($query){
            return $query-> where('to',
            Auth::guard('admin')->user()->id
    ...
    public function countUnreadEmail(){
        return Email::with(['sender','receiver','EmployeeReceiver','EmployeeSender']) -> where('to',
        Auth::guard('admin')->user()->id
        )->whereNull('soft_deleted_by_receiver')->where('status','unread')->latest()->count();
    ...
    public function destroySentEmail($id){
        ...
        if($email->to == Auth::guard('admin')->user()->id){
